...
static void reset(struct drm_crtc *crtc)
{
	...
	/* disable lcdc and wait for it to stop */
	tilcdc_clear(dev, LCDC_RASTER_CTRL_REG, LCDC_RASTER_ENABLE);
	tilcdc_write(dev, LCDC_MASKED_STAT_REG, 0xffffffff);
	do {
		reg = tilcdc_read(dev, LCDC_RAW_STAT_REG);
		msleep(5);
	} while (--retry && !(reg & LCDC_FRAME_DONE));
