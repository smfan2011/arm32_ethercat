...
#include "./encoder-sii9022-audio.h"
...
static int sii902x_get_modes(struct drm_connector *connector)
{
	...
	if (edid) {
		...
		sii902x->hdmi_mode = drm_detect_hdmi_monitor(edid);
		dev_info(&sii902x->i2c->dev, "hdmi_mode = %d", sii902x->hdmi_mode);
		kfree(edid);
	}
	else
		dev_err(&sii902x->i2c->dev, "sii902x driver failed to get edid\n");
...
static void sii902x_bridge_enable(struct drm_bridge *bridge)
{
	...
	/* 0 = DVI, 1 = HDMI */
	ret = regmap_update_bits(sii902x->regmap, SII902X_SYS_CTRL_DATA, 1 << 0, sii902x->hdmi_mode ? 1 : 0)	;
	if (ret) {
		dev_err(&sii902x->i2c->dev, "failed to set DVI/HDMI mode\n");
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	/* ADD */
	regmap_write(sii902x->regmap, SII902X_INT_ENABLE, 0x3);
	...
	bridge = &sii902x->bridge;

	sii9022_hdmi_codec_register(&client->dev, sii902x->regmap);
